-- Applied to live DB 2026-08-15 as 20260815162003_clinic_generator_capacity_and_smart_cap_dates
-- 1) clinic_day_count: helper counting real (non-cancelled, non-away, non-protected)
--    fellow assignments for a clinic on a date, so capacity is a true per-day ceiling.
-- 2) generate_clinic_schedule: enforces fellow_capacity everywhere, and for monthly-capped
--    slots picks "designated" dates smartly (prefers dates when the fallback provider is away).

create or replace function public.clinic_day_count(p_date date, p_site text, p_provider uuid, p_pname text)
returns integer
language sql
...
as $function$
  ...
  where cr.rotation_date = p_date
    and cr.status <> 'cancelled'
    and cr.is_away = false
    and cr.is_protected = false
    and cr.site_code = p_site
    and cr.supervisor_id is not distinct from p_provider
    and (p_provider is not null or cr.provider_name is not distinct from p_pname);
$function$;
