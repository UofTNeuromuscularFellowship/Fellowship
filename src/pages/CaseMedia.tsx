...
function UploadForm({
  ...
  const consentReady = !needsConsent || (patientName.trim().length > 0 && signature !== null)
  ...
  async function submit() {
    ...
    try {
      ...
      if (needsConsent && signature) {
        try {
          await saveConsent(created.id, { patientName, wording, signature }, authorId)
        } catch (e) {
    ...
    <div className="space-y-3">
      ...
      <label className="block">
        ...
        <input
          ...
          className="mt-1 block w-full text-sm text-ink file:mr-3 file:rounded-md file:border file:border-line file:bg-surface file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-ink"
        />
      </label>
      ...
      {needsConsent && (
        <div className="rounded-md border border-accent/40 bg-accent-soft/20 px-4 py-3">
          ...
          <p className="mt-1 text-sm text-ink">
            A photograph or recording of a patient goes up only with their signed permission. Read
            the wording below to them, then have them sign.
          </p>
      ...
      {!ready && !busy && (
        <p className="text-xs text-muted">
          ...
          {needsConsent ? ', and the patient\u2019s name and signature' : ''}.
        </p>
      )}
    </div>
  )
        ...
        <>
          ...
          {open && (
            <div className="mt-3 space-y-2 rounded-md border border-line bg-surface px-4 py-3">
              ...
              <p className="whitespace-pre-line text-sm text-ink">{record.wording}</p>
              <p className="text-sm text-ink">
...
function CaseView({
  ...
  return (
    <Card>
        ...
        action={
          mayEdit ? (
            <div className="flex shrink-0 gap-2">
              ...
              <button onClick={() => void remove()} className="text-xs font-semibold text-red-600 hover:underline">
                Delete
              </button>
            </div>
          ) : undefined
      ...
      {/* ---- drawing toolbar ---- */}
      {editing && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-line bg-paper/60 px-5 py-2.5">
          ...
          <button
            ...
            className="rounded-md border border-line px-2.5 py-1.5 text-xs font-semibold text-muted hover:text-ink disabled:opacity-40"
          >
            Undo
          </button>
          <button
          ...
          <div className="space-y-2">
            ...
            {mayEdit && (
              <button
                ...
                disabled={busy}
                className="rounded-md border border-line px-3 py-1.5 text-xs font-semibold text-accent disabled:opacity-40"
              >
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          Teaching images and clips — waveforms, ultrasound, biopsy and examination findings — with
