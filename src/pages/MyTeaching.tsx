...
interface NamedFeedbackRow extends FeedbackRow {
  fellow_name: string
}
...
export default function MyTeaching() {
  ...
  async function load() {
    ...
    let q = supabase
      ...
      .select('id, session_date, start_time, end_time, topic, provider_name, provider_id, zoom_link, delivered_at, is_break')
      .eq('is_break', false)
      .order('session_date')
    if (!isDirector && profile) q = q.eq('provider_id', profile.id)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          {isDirector ? 'All teaching sessions, delivery status, and feedback' : 'Your assigned sessions, delivery log, and feedback'}
        </p>
      </div>
...
function SessionRow({ s, isDirector }: { s: Session; isDirector: boolean }) {
  return (
